...
export default function RegisterPage() {
    ...
    return (
        <AuthFormContainer title="Buat Akun" goHome={() => navigate('/') }>
            <form onSubmit={handleSubmit}>
                {error && <p className="text-red-500 text-center text-sm mb-4">{error}</p>}
                <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap</label>
                    <input onKeyDown={handleKeyDown} type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} required className="w-full p-3 bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C89F74]" />
                </div>
                <div className="mb-4">
